...
import { 
  ...
  Trash2, Printer, CheckCircle, XCircle, AlertCircle,
  ArrowUpRight, ArrowDownRight, DollarSign, Calendar, 
  ChevronLeft, ChevronRight, Filter, LogOut, Lock, Users, Globe, Briefcase, Link, MessageSquare, Upload, Star, FileCheck, TrendingUp, Camera, Box, Database, Calculator, Pencil, X, File, Download, Wallet, MapPin, Palette
} from "lucide-react";
import { 
...
function KPICard({ title, value, customValue, color, icon, bg = "bg-[#0f0f0f]" }: any) {
  return (
    <div className={`${bg} border border-white/5 p-6 rounded-[32px] flex flex-col justify-between aspect-video relative overflow-hidden group`}>
      ...
      <p className={`text-2xl font-black tracking-tighter relative z-10 ${color}`}>{customValue || formatCurrency(value)}</p>
    </div>
  );
}
...
function LoginView() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-black p-8">
            <form onSubmit={handleLogin} className="w-full max-w-sm bg-[#0f0f0f] border border-white/10 p-12 rounded-[48px] space-y-6 shadow-2xl relative text-white">
                <div className="absolute top-0 left-0 w-full h-1.5 bg-purple-600" />
                <div className="text-center mb-10"><Lock className="mx-auto text-purple-500 mb-6" size={48} /><h2 className="font-black uppercase tracking-[0.3em] text-xl">Lenz Hub</h2></div>
                <input type="email" placeholder="E-mail" value={email} onChange={e => setEmail(e.target.value)} className="input-field bg-black text-white mb-4" required />
                <input type="password" placeholder="Senha" value={password} onChange={e => setPassword(e.target.value)} className="input-field bg-black text-white" required />
                <button disabled={loading} className="w-full bg-purple-600 text-white font-black py-5 rounded-2xl tracking-[0.2em] uppercase text-xs shadow-lg active:scale-95 transition-all mt-6">{loading ? "Validando..." : "Entrar"}</button>
            </form>
        </div>
    );
}
...
function DashboardView({ propostas, financeiro, equipamentos }: any) {
    const stats = useMemo(() => {
      ...
      const valorEmpresa = saldoLiquido + patrimonio;
      return { totalAprovado, saldo: saldoLiquido, valorEmpresa, patrimonio, dinheiroNaMesa };
          ...
          <div className="h-[400px] bg-[#0f0f0f] border border-white/5 p-8 rounded-[40px] shadow-2xl">
              ...
              <ResponsiveContainer width="100%" height="85%">
                  <AreaChart data={chartData}>
                      <defs>
                        <linearGradient id="colorEnt" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#9333ea" stopOpacity={0.3}/><stop offset="95%" stopColor="#9333ea" stopOpacity={0}/></linearGradient>
                      </defs>
...
function FinanceiroView({ financeiro, refresh, session, propostas, onPrint }: any) {
    ...
    const [novo, setNovo] = useState<any>({ 
      ...
      imposto_porcentagem: 6, imposto_retido: 0, 
      proposta_id: "", data: new Date().toISOString().slice(0, 10),
      custos_extras: [], split_regras: { socio_pct: 100, empresa_pct: 0 }
    });

    useEffect(() => {
        if(novo.valor && novo.imposto_porcentagem >= 0) {
            setNovo((prev: any) => ({...prev, imposto_retido: (prev.valor * prev.imposto_porcentagem)/100}));
        }
    }, [novo.valor, novo.imposto_porcentagem]);

    useEffect(() => {
        if(novo.proposta_id) {
            const p = propostas.find((x:any) => x.id === novo.proposta_id);
            if(p) {
    ...
    const resumo = filtradas.reduce((acc:any, t:any) => {
        if(t.tipo === 'entrada') acc.ent += Number(t.valor); else acc.sai += Number(t.valor); return acc;
    }, { ent: 0, sai: 0 });

    const calculo = useMemo(() => {
       const v = Number(novo.valor||0);
       const i = Number(novo.imposto_retido||0);
       const c = novo.custos_extras?.reduce((acc:number,x:any)=>acc+Number(x.valor),0)||0;
       return { totalCustos: c, lucroLiquido: (v-i-c) > 0 ? v-i-c : 0 };
    }, [novo]);

    // FILTER FIX: Apenas propostas do MÊS/ANO ativo aparecem no select
    const propostasDisponiveis = useMemo(() => {
        return propostas.filter((p:any) => p.data.startsWith(`${anoAtivo}-${mesAtivo}`));
    }, [propostas, anoAtivo, mesAtivo]);
    ...
    return (
        <div className="space-y-8 animate-in fade-in text-white">
            ...
            <div className="bg-[#0f0f0f] border border-white/5 rounded-[40px] p-8 space-y-6 shadow-2xl">
                 <h3 className="text-xs font-black uppercase text-purple-400 flex items-center gap-2"><Wallet size={16}/> Lançamento Mês {mesAtivo}/{anoAtivo}</h3>
                 <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                     <select className="input-field col-span-1" value={novo.proposta_id || ""} onChange={e=>setNovo({...novo, proposta_id: e.target.value})}>
                         <option value="">Vincular Proposta ({propostasDisponiveis.length} Disp.)</option>
                         {propostasDisponiveis.map((p:any)=><option key={p.id} value={p.id}>{p.projeto} - {p.cliente}</option>)}
                     </select>
                     <input className="input-field col-span-2" placeholder="Descrição" value={novo.descricao} onChange={e=>setNovo({...novo, descricao:e.target.value})} />
                     <div className="flex bg-black rounded-xl border border-white/10 p-1 col-span-1"><button onClick={()=>setNovo({...novo, tipo: 'entrada'})} className={`flex-1 rounded-lg text-xs font-black uppercase ${novo.tipo==='entrada' ? 'bg-green-600 text-white' : 'text-neutral-500'}`}>Entrada</button><button onClick={()=>setNovo({...novo, tipo: 'saida'})} className={`flex-1 rounded-lg text-xs font-black uppercase ${novo.tipo==='saida' ? 'bg-red-600 text-white' : 'text-neutral-500'}`}>Saída</button></div>
                 </div>
        ...
        <div className="space-y-6 max-w-lg mx-auto text-white animate-in slide-in-from-right">
            ...
            <div className="bg-[#0f0f0f] border border-white/5 p-8 rounded-[40px] space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <input className="input-field" placeholder="CNPJ" value={editing.cnpj || ''} onChange={e=>setEditing({...editing, cnpj: e.target.value})} />
                    <input className="input-field" placeholder="Endereço / UF" value={editing.endereco || ''} onChange={e=>setEditing({...editing, endereco: e.target.value})} />
                </div>
    ...
    if(editing) {
       ...
       return (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
                   ...
                   <div className="bg-[#0f0f0f] p-8 rounded-[40px] space-y-4 border border-white/5">
                       ...
                       <div className="border-t border-white/10 pt-4 space-y-2">
                           <div className="flex justify-between items-center mb-1">
                               <p className="label-field">Tabela de Preço (Itens)</p>
                           </div>
                           
                           {/* HEADER DA TABELA - FIX CAIXA FANTASMA */}
                           <div className="flex gap-2 mb-1 px-1">
                               <span className="text-[9px] font-bold text-gray-500 w-14 text-center">QTD</span>
                               <span className="text-[9px] font-bold text-gray-500 flex-1">DESCRIÇÃO</span>
                               <span className="text-[9px] font-bold text-gray-500 w-28 text-right">VALOR UNIT.</span>
                               <span className="w-6"></span>
                           </div>

                           {(editing.itens||[]).map((it:any,i:number)=>(
                               <div key={i} className="flex gap-2 items-center bg-black/20 p-1 rounded-lg">
                                   <input 
                                       type="number"
                                       className="bg-[#1a1a1a] border border-[#333] rounded-lg p-2 text-white outline-none w-14 text-center text-xs focus:border-purple-600" 
                                       placeholder="1" 
                                       value={it.qtd} 
                                       onChange={e=>{const cp=[...editing.itens];cp[i].qtd=Number(e.target.value);setEditing({...editing, itens:cp})}}
                                   />
                                   <input 
                                       className="bg-[#1a1a1a] border border-[#333] rounded-lg p-2 text-white outline-none flex-1 text-xs focus:border-purple-600" 
                                       placeholder="Descrição do Serviço/Equipamento" 
                                       value={it.descricao} 
                                       onChange={e=>{const cp=[...editing.itens];cp[i].descricao=e.target.value;setEditing({...editing, itens:cp})}}
                                   />
                                   <input 
                                       type="number"
                                       className="bg-[#1a1a1a] border border-[#333] rounded-lg p-2 text-white outline-none w-28 text-right text-xs focus:border-purple-600" 
                                       placeholder="R$ 0,00" 
                                       value={it.valor} 
                                       onChange={e=>{const cp=[...editing.itens];cp[i].valor=Number(e.target.value);setEditing({...editing, itens:cp})}}
                                   />
                                   <button onClick={()=>setEditing({...editing, itens:editing.itens.filter((_:any,x:number)=>x!==i)})} className="text-neutral-500 hover:text-red-500 w-6 flex justify-center">
                                       <Trash2 size={16}/>
                                   </button>
                               </div>
                           ))}
                           
                           <button onClick={()=>setEditing({...editing, itens:[...(editing.itens||[]), {descricao:'', qtd:1, valor:0}]})} className="w-full text-[10px] font-black uppercase text-purple-400 bg-white/5 py-3 rounded-xl hover:bg-white/10 mt-2 flex items-center justify-center gap-2">
               ...
               {/* PREVIEW A4 NA TELA (CORREÇÃO DE FORMATAÇÃO DO A4 E ESPAÇAMENTOS) */}
               <div className="bg-[#111] border border-white/5 p-4 rounded-[40px] flex justify-center items-start overflow-auto h-full hidden lg:flex">
                   <div className="bg-white text-black w-[210mm] min-h-[297mm] p-[15mm] relative shadow-2xl flex flex-col justify-between origin-top scale-[0.55] overflow-hidden">
                       ...
                       <div className="relative z-10">
                            {/* HEADER CORRIGIDO: SEM DATA NO TOPO */}
                            ...
                            <h1 className="font-black uppercase mb-4 leading-none tracking-tighter break-words" style={{fontSize: `${editing.tamanho_titulo||48}px`, color: editing.cor_titulo||'#000'}}>{editing.projeto}</h1>
                            <p className="font-bold uppercase tracking-[0.2em] mb-12" style={{fontSize: `${editing.tamanho_subtitulo||14}px`, color: editing.cor_subtitulo||'#666'}}>{editing.local_evento || 'DATA / LOCAL À DEFINIR'}</p>
                            
                            {/* CAIXA CONTEXTO E ESCOPO: PREVINE CORTE */}
                            ...
                            {/* TABELA CORRIGIDA: IMPEDE CORTE E MANTEM ESPAÇAMENTO */}
                            <div className="mt-8">
                                {editing.visualizacao === 'simples' ? (
                                    ...
                                    <table className="w-full text-left" style={{fontSize: `${editing.tamanho_corpo||14}px`, color: editing.cor_corpo||'#000'}}><tbody>{(editing.itens||[]).map((it:any,i:number)=>(<tr key={i} className="border-b border-neutral-100 last:border-0"><td className="py-3 font-bold uppercase w-1 whitespace-nowrap align-top pr-4">{it.qtd}x</td><td className="py-3 font-bold uppercase break-words align-top">{it.descricao}</td><td className="py-3 font-black text-right whitespace-nowrap align-top w-40">{formatCurrency(it.valor*it.qtd)}</td></tr>))}</tbody></table>
                                )}
                            </div>
                       </div>
                       ...
                       <div className="mt-auto border-t-2 border-black pt-4 flex justify-between items-end relative z-10">
                            ...
                            <div className="text-right">
                                <p className="text-[10px] font-bold uppercase text-neutral-400">Emissão: {formatarDataSimples(editing.data)}</p>
                                <p className="text-[10px] font-bold uppercase text-neutral-400">CNPJ: {CNPJ_LENZ} • Validade: 7 dias</p>
                            </div>
                       </div>
                   </div>
               </div>
           ...
           {printData.type === 'proposta' ? (
             <div className="flex flex-col h-[280mm] justify-between relative overflow-hidden bg-white">
                <div className="absolute inset-0 pointer-events-none z-0 flex items-center justify-center">
                    {/* Imagem de Fundo (z-0 negativo para não cobrir) */}
                ...
                <div className="relative z-10 bg-white/0">
                     ...
                     <div className="mb-14">
                        <h1 className="font-black uppercase mb-4 leading-none tracking-tighter break-words" style={{ fontSize: `${printData.data.tamanho_titulo||48}px`, color: printData.data.cor_titulo||'#000' }}>{printData.data.projeto}</h1>
                        <p className="font-bold uppercase tracking-[0.2em] text-black" style={{ fontSize: `${printData.data.tamanho_subtitulo||14}px`, color: printData.data.cor_subtitulo||'#666' }}>{printData.data.local_evento || 'LOCAL À DEFINIR'}</p>
                     </div>
                     ...
                     <div>
                         ...
                         <div>
                             ...
                             {/* FORMATO TABELA CORRIGIDO (Break-Words, Align-Top) */}
                             <table className="w-full text-left" style={{ fontSize: `${printData.data.tamanho_corpo||14}px`, color: printData.data.cor_corpo||'#000' }}>
                                <tbody>{printData.data.itens.map((it:any,i:number)=>(<tr key={i} className="border-b border-neutral-200"><td className="py-3 font-bold uppercase w-1 whitespace-nowrap align-top pr-4">{it.qtd}x</td><td className="py-3 font-bold uppercase break-words align-top">{it.descricao}</td><td className="py-3 font-black text-right whitespace-nowrap align-top w-40">{formatCurrency(it.valor*it.qtd)}</td></tr>))}</tbody>
                             </table>
                         </div>}
                     </div>
                </div>
                <div className="border-t-2 border-black pt-4 flex justify-between items-end mt-auto relative z-10 bg-white">
                     ...
                     {/* RODAPÉ BLINDADO (SEM HEADER DATE) */}
                     <div className="text-right text-[10px] uppercase font-bold text-neutral-400">
                        <p>Emissão: {formatarDataSimples(printData.data.data)}</p>
                        <p>CNPJ: {CNPJ_LENZ} • Validade: 7 dias</p>
                     </div>
                </div>
             </div>
           ) : (
